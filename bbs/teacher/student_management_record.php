		...
		<table>
            ...
            for($i=1; $i<count($r); $i++) {
                ...
                <tr>
                    ...
                    <td>
                        <div class="hw_manegement_btn <?php if(!$res['hm_mg']) echo "disable";?>"><a href="homework_management_personal.php?s_id=<?=$r[$i][1]?>&d_uid=<?=$_GET['d_uid']?>&c_uid=<?=$_GET['c_uid']?>&class_name=<?=$class_name?>&student=<?=$r[$i][2]?>">숙제관리</a></div>
                        ...
                        <div class="scoring_list_btn"><a href="scoring_list.php?s_id=<?=$r[$i][1]?>&d_uid=<?=$_GET['d_uid']?>&c_uid=<?=$_GET['c_uid']?>">채점목록</a></div>
                    </td>
                </tr>
                <?
            }
