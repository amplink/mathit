...
for($i=0; $i<count($student); $i++) {
    $sql = "INSERT INTO `teacher_score` (`client_id`, `class`, `year`, `quarter`, `d_order`, `test_genre`, `title`, `date`, `standard`, `sub_score1`, `sub_score2`, `score1`, `score2`, `teacher`, `student_id`, `student`, `event_time`) 
VALUES ('$_SESSION[client_no]', '$class', '$year', '$quarter', '$d_yoie', '$test_genre', '$title', '$date', '$standard', '$sub_score1', '$sub_score2', '$score_add1[$i]', '$score_add2[$i]', '$teacher', '$student_id[$i]', '$student[$i]', CURRENT_TIMESTAMP);";
    sql_query($sql);
    //echo $sql;
}
