...
if($section_1[0][0]) {
    ...
    for($i=0; $i<$section_size[0]; $i++) {
        if($section_1[3][$i]) { // 기존 데이터가 있다면
            ...
            $sql = "UPDATE `answer_master` set `seq` = '$i', `item_number`='".$section_1[0][$i]."', `answer_image` = '".$answer_img."', `explain_image`='".$explain_img."', `new`=1, `event_time`=CURRENT_TIMESTAMP where `answer_id`='$answer_id';";
            sql_query($sql);
        }else { // 새로운 데이터라면
...
if($section_2[0][0]) {
    ...
    for($i=0; $i<$section_size[1]; $i++) {
        if($section_2[3][$i]) { // 기존 데이터가 있다면
            ...
            $sql = "UPDATE `answer_master` set `seq` = '$i', `item_number`='".$section_2[0][$i]."', `answer_image` = '".$answer_img."', `explain_image`='".$explain_img."', `new`=1, `event_time`=CURRENT_TIMESTAMP where `answer_id`='$answer_id';";
            sql_query($sql);
        }else { // 새로운 데이터라면
            $answer_id = rand(1, 22222).":".date("mds");
...
if($section_3[0][0]) {
    ...
    for($i=0; $i<$section_size[2]; $i++) {
        if($section_3[3][$i]) { // 기존 데이터가 있다면
            ...
            $sql = "UPDATE `answer_master` set `seq` = '$i', `item_number`='".$section_3[0][$i]."', `answer_image` = '".$answer_img."', `explain_image`='".$explain_img."', `new`=1, `event_time`=CURRENT_TIMESTAMP where `answer_id`='$answer_id';";
            sql_query($sql);
        }else { // 새로운 데이터라면
            $answer_id = rand(1, 22222).":".date("mds");
...
if($section_4[0][0]) {
    ...
    for($i=0; $i<$section_size[3]; $i++) {
        if($section_4[3][$i]) { // 기존 데이터가 있다면
            ...
            if($section_4[1]['tmp_name'][$i]) { // answer_img가 변경 됐다면
                $answer_img = "";
            ...
            $answer_id = $section_4[3][$i];

            $sql = "UPDATE `answer_master` set `seq` = '$i', `item_number`='".$section_4[0][$i]."', `answer_image` = '".$answer_img."', `explain_image`='".$explain_img."', `new`=1, `event_time`=CURRENT_TIMESTAMP where `answer_id`='$answer_id';";
            sql_query($sql);
        }else { // 새로운 데이터라면
            $answer_id = rand(1, 22222).":".date("mds");
